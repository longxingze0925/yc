...
用户已于 2026-07-21 确认 M0 文档冻结决策并授权开始开发，并于 2026-07-22 接受登录设备身份 ADR、确认 `0001_initial_schema.sql` 未进入共享环境。项目当前进入 V1 Core 的 M1-M7 实施阶段，账号身份 Schema 修订必须重写 `0001`，并按任务清单依赖顺序交付可运行闭环。

首个最小可运行闭环：

```text
主控端 -> 登录/发起连接 -> 信令协商 -> P2P/Relay 建链 -> 被控端采集画面 -> 主控端显示 -> 输入回传
```
